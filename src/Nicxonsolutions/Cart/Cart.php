...
use Nicxonsolutions\Cart\Exceptions\InvalidConditionException;
use Nicxonsolutions\Cart\Exceptions\InvalidItemException;
use Nicxonsolutions\Cart\Helpers\Helpers;
use Nicxonsolutions\Cart\Validators\CartItemValidator;
use Nicxonsolutions\Cart\Exceptions\UnknownModelException;

/**
 * Class Cart
 * @package Nicxonsolutions\Cart
 */
class Cart
{
    ...
    public function addItemCondition($productId, $itemCondition)
    {
        if ($product = $this->get($productId)) {
            $conditionInstance = "\\Nicxonsolutions\\Cart\\CartCondition";

            if ($itemCondition instanceof $conditionInstance) {
                // we need to copy first to a temporary variable to hold the conditions
                // to avoid hitting this error "Indirect modification of overloaded element of Nicxonsolutions\Cart\ItemCollection has no effect"
    ...
    public function condition($condition)
    {
        ...
        if (!$condition instanceof CartCondition) throw new InvalidConditionException('Argument 1 must be an instance of \'Nicxonsolutions\Cart\CartCondition\'');
